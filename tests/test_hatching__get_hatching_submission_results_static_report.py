...
def test_raises_exception_and_error_logged(hatching_service_instance, caplog):
    ...
    hatching_service_instance.triage_client.static_report = MagicMock(side_effect=exc)

    sample_id = "1"
    with pytest.raises(RetryError):
        results = hatching_service_instance._get_hatching_submission_results_static_report(
            sample_id
        )
    ...
    assert (
        f"Excpetion retrieving the static report for sample_id: {sample_id}"
        in caplog.text
    )
    assert len(errors) == 2
