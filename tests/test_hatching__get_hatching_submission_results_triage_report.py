...
def test_raises_exception_and_error_logged(hatching_service_instance, caplog):
    ...
    hatching_service_instance.triage_client.task_report = MagicMock(side_effect=exc)
    ...
    assert (
        f"Exception retrieving the dynamic triage report for sample_id: {sample_id}  task_id: {task_id}"
        in caplog.text
    )
    assert len(errors) == 2
